...
function buildCloseBeforeLabel(
  ...
  return surface === "mobile" ? text.closeTabsAbove : text.closeToLeft;
}
...
function buildCloseAfterLabel(
  ...
  return surface === "mobile" ? text.closeTabsBelow : text.closeToRight;
}
...
function getCloseButtonTestId(tab: WorkspaceTabDescriptor): string {
  ...
  if (tab.target.kind === "preview") {
    return `workspace-preview-close-${encodeFilePathForPathSegment(tab.target.scriptName)}`;
  }
...
export function buildWorkspaceTabMenuEntries(
  ...
  entries.push({
    kind: "item",
    key: "close-before",
    label: buildCloseBeforeLabel(surface, text),
  ...
  entries.push({
    kind: "item",
    key: "close-after",
    label: buildCloseAfterLabel(surface, text),
  ...
  if (tab.target.kind === "agent") {
    ...
    entries.push({
      kind: "item",
      key: "reload-agent",
      label: text.reloadAgent,
      icon: "rotate-cw",
      tooltip: text.reloadAgentTooltip,
