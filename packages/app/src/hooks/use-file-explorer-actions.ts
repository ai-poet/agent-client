...
export function useFileExplorerActions(params: { serverId: string } & FileExplorerWorkspaceScope) {
    ...
    async (
      ...
      if (!normalizedWorkspaceRoot) {
        updateExplorerState((state) => ({
          ...state,
          isLoading: false,
          lastError: text.workspaceUnavailable,
          pendingRequest: null,
        }));
        return false;
      }
      ...
      if (!client) {
        updateExplorerState((state) => ({
          ...state,
          isLoading: false,
          lastError: text.hostNotConnected,
          pendingRequest: null,
        }));
        return false;
      }
      ...
      try {
        ...
        updateExplorerState((state) => ({
          ...state,
          isLoading: false,
          lastError: error instanceof Error ? error.message : text.failedListDirectory,
          pendingRequest: null,
        }));
        return false;
      }
  ...
  const requestFileDownloadToken = useCallback(
    async (path: string) => {
      if (!normalizedWorkspaceRoot) {
        throw new Error(text.workspaceUnavailable);
      }
      if (!client) {
    ...
    [client, normalizedWorkspaceRoot, text.hostNotConnected, text.workspaceUnavailable],
  );
