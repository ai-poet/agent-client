...
| `crates/waku-agent/api/src/prompt_cache.rs` (new), `crates/waku-agent/api/src/providers/anthropic.rs`, `crates/waku-agent/api/src/lib.rs` | vendored engine, recorded departure: Claude Code's prompt-cache breakpoints (last tool, system prompt, last message, the user message before it) on every Messages request; one `pub mod` line and one call in `build_request` | ~5 + new file |
